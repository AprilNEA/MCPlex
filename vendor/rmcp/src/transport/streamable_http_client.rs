// MCPlex vendor change from rmcp 3.1.2: forward explicitly attached
// Mcp-Param-* headers when this transport is used as a proxy hop.
use std::{
    borrow::Cow,
...
fn build_request_headers(
    ...
    if *version >= ProtocolVersion::STANDARD_HEADERS
        ...
        && let Some(forwarded) = request.request.extensions().get::<http::HeaderMap>()
    {
