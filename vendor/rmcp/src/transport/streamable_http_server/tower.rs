// MCPlex vendor changes from rmcp 3.1.2: validate complete modern request
// metadata and unsupported versions before response streaming. MCPlex runs the
// existing request-scoped SSE path so disconnect can cancel work before the
// first MCP message.
use std::{
    borrow::Cow,
    collections::HashMap,
...
fn validate_request_protocol_version_meta(
    ...
    let missing = meta.missing_required_keys(&meta_version);
    if !missing.is_empty() {
        return Err(invalid_params_jsonrpc_response(
            Some(request.id.clone()),
            format!(
    ...
    M: SessionManager,
{
    ...
    async fn serve_negotiated_request_directly(
        ...
        if let Some(requested) = request.request.get_meta().protocol_version() {
            let supported = service.supported_protocol_versions();
            if !supported.contains(&requested) {
                return jsonrpc_message_response(
                    ServerJsonRpcMessage::Error(JsonRpcError::new(
                        Some(request.id.clone()),
                        ErrorData::unsupported_protocol_version(requested, &supported),
                    )),
                    true,
                );
            }
        ...
        if !self.config.json_response {
            return Ok(self.stateless_sse_response(None, receiver, request_ct));
        }
